...
object Registration extends Controller {
    ...
    implicit request =>
      startForm.bindFromRequest.fold(
        ...
        email => {
          ...
          UserService.findByEmailAndProvider(email, UsernamePasswordProvider.UsernamePassword) map {
            case Some(user) => {
  ...
  private def executeForToken(token: String, isSignUp: Boolean, f: Token => Result): Result = {
    Async {
      UserService.findToken(token).map {
  ...
  def handleResetPassword(token: String) = Action {
    implicit request =>
      executeForToken(token, false, {
        t => changePasswordForm.bindFromRequest.fold(
          errors => {
          ...
          p => {
            ...
            Async(result.map(Redirect(RoutesHelper.login()).flashing(_)))
          }
        )
      })
  }
